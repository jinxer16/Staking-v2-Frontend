...
const Deposit:React.FC<DepositModalProps> = forwardRef((props, ref) => {
  ...
  let defaultTokenBalance = Math.floor(useTokenBalance(pool.stakeToken[DEFAULT_CHAIN_ID]) * 100000) / 100000
  ...
  const defaultTokenBalalnceFormatted = defaultTokenBalance 
    ? `${defaultTokenBalance.toLocaleString(undefined,{maximumFractionDigits: 5})} ${pool.stakeTokenSymbol}`
    : `0.000000000 ${pool.stakeTokenSymbol}`

  const [tokenBalance, setTokenBalance] = React.useState(defaultTokenBalance);
  const [tokenBalanceFormatted, setTokenBalanceFormatted] = React.useState(defaultTokenBalalnceFormatted);

  useEffect(() => {
      if (account) {
        const fetchETHBalance = async (_account, _provider) => {
          const res = await _provider.getBalance(_account)
          setTokenBalance(Math.floor(getBalanceNumber(new BigNumber(res.toString()), 18) * 10000) / 10000)
        }
  ...
  useEffect(() => {
    const _tokenBalanceFormatted = tokenBalance
      ? `${tokenBalance.toLocaleString(undefined,{maximumFractionDigits: 5})} ${pool.stakeTokenSymbol}`
      : `0.000000000 ${pool.stakeTokenSymbol}`
        ...
        <button disabled={pendingTx} onClick={async () => {
          if (tokenAmount <= tokenBalance) {
            ...
            try {
              ...
              let stakeStatus = 0
              if (pool.isNativePool) {
                stakeStatus = await onStake(tokenAmountInBigNum.toString(10), tokenAmountInBigNum.toString(10))
              } else {
