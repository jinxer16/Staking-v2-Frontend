...
import { getLpPrice, getTokenMarketInfo, getTokenPrice } from 'utils/priceHelpers'
import { DEFAULT_CHAIN_ID, FIBO_TOKEN_ADDRESS, NATIVE_TOKEN_ADDRESS } from 'config'
...
export const usePoolFromName = (name: string): PoolConfig => {
    const pool = useSelector(( state: State ) => state.pools.data.find((f) => f.name == name))
    return pool!
}
...
export const usePoolUser = (name: string) => {
    ...
    return {
        ...
        stakedBalance: pool.userData? pool.userData.stakedBalance : new BigNumber(0),
        earned: pool.userData? pool.userData.earned : new BigNumber(0),
        lastDeposited: pool.userData && pool.userData.lastDeposited ? pool.userData.lastDeposited : 0,}
}

export const useTokenPrice = (name: string, isLpToken: boolean, chainId: number) => {
...
export const useETHPrice = (chainId: number) => {
    const lpPool = lpPools.find((f) => f.name === QuoteToken.WMATIC) // WMATIC-USDC pool
...
export const useTokenMarketInfo = (tokenAddress: string) => {
    const [marketInfo, setMarketInfo] = useState({totalSupply: new BigNumber(0), burnAmount: new BigNumber(0), circulateSupply: new BigNumber(0)})
    useEffect(() => {
    ...
    useEffect(() => {
        const fetchSupply = async () => {
            ...
            if (totalRewardPaid.data.data.settingEntities.length > 0) {
                setSupply(new BigNumber(totalRewardPaid.data.data.settingEntities[0].totalPaid))
            }
        }
    ...
    useEffect(() => {
        ...
        const fetchTotalLocked = async (_pools: PoolConfig[]) => {
            ...
            for (let i = 0; i < _pools.length; i++) {
                ...
                if (totalSupply) {
                    const totalSupplyInNum = getBalanceNumber(totalSupply, pool.stakeTokenDecimal)
                    value = value.plus(getDecimalAmount(new BigNumber(totalSupplyInNum * tokenPrice)))
                }
            }
