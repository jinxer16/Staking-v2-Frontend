...
import { fetchPoolUserAllowances, fetchPoolUserStakedBalance, fetchPoolUserEarned, fetchPoolUserLastDeposited } from './fetchPoolsUser'
    ...
    reducers: {
      setPoolsPublicData: (state, action) => {
          ...
          state.data = state.data.map((pool) => {
              const livePoolData = livePoolsData.find((f) => f.address[DEFAULT_CHAIN_ID] === pool.address[DEFAULT_CHAIN_ID])
